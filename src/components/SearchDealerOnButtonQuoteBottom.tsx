...
@observer
class SearchDealerOnButtonQuoteBottom extends Component<Props, {}> {

    displayQuoteYourTires = () => {

    if (this.props.mainStore.lang == 'fr')
        return ('Obtenez un devis pour vos pneus ici');

    return (STRCONSTANT.common.default.quote_tires);
          ...
          trigger={
              <a  
                ...
                onClick=
                  { () =>  
                    {
                      this.props.mainStore.trackingEventDealer('Quote Your Tires - Bottom');
                      this.props.mainStore.openSearchDealerModal('QuoteYourTires')
                    }
                  }
                target="_blank" title="">
                <span className="imgin"></span>
                <span  id="change-your-dealer">
                    <p style={{paddingRight: "25px", paddingLeft: "25px"}}>
                  { this.displayQuoteYourTires() }
                    </p>
                </span>
              </a>
          }
...
export default typedInject('mainStore')(SearchDealerOnButtonQuoteBottom);